...
5. **`prototypes/planets.lua`** - Deep-copy base planet prototypes for each variant (following Team Starts pattern at `~/src/team-starts/prototypes/planets.lua`).
...
Add `on_technology_effects_reset` and `on_force_reset` handlers to re-lock nauvis and re-unlock team planet variants (following Team Starts pattern at `~/src/team-starts/scripts/universe.lua` lines 89-127).
